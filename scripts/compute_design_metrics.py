"""Compute design-document quality metrics for one model run."""
...
from evaluation_metrics import (
    ...
    compute_design_metrics_for_problem,
    display_path,
    problem_dirs,
    resolve_path,
    write_csv,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute design-document metrics for HumanEval-X C++ run outputs.")
...
def main() -> int:
    ...
    for problem_dir in selected_dirs:
        metrics = compute_design_metrics_for_problem(problem_dir)
        write_json(problem_dir / "results" / "design_metrics.json", metrics)
        rows.append(metrics)

    fields = ["problem_id", "status", *DESIGN_METRIC_FIELDS]
    write_csv(output_dir / "design_metrics_summary.csv", rows, fields)

    print(f"Wrote design metrics for {len(rows)} problems.")
    print(f"Wrote summary: {display_path(output_dir / 'design_metrics_summary.csv')}")
    return 0
